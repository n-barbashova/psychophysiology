...
for ID in IDs:
    for run in runs:
        ...
        data_1_save = data_1[['ECG', "EVENT"]]
        data_2_save = data_2[['ECG', "EVENT"]]
        data_3_save = data_3[['ECG', "EVENT"]]
        data_4_save = data_4[['ECG', "EVENT"]]
        ...
        encoding_downsample = data_1_save[::20].copy()        print("Starting run num:", run)
        subject.append(ID) #subject list
        runli.append(run)  #run list - in the end you have info to make a spreadsheet
        current_dir = rawdata + "/" + ID
        current_file = "fmcc_sub" + ID + "_task_000" + str(run) + ".csv"
        ...
        for name, df in data_dict.items():
            ...
            # Reorder columns to have timepoint, ECG, Event code
            encoding_downsample = encoding_downsample[['timepoint', 'ECG', 'EVENT']]
